...
from dataloaders.brats19 import IAVS, SagittalToAxial, RandomCrop, RandomRotFlip, SkeletonTransform, ToTensor, TwoStreamBatchSampler 

# from topolosses.losses import BettiMatchingLoss, CLDiceLoss, HutopoLoss, MosinLoss, TopographLoss, WarpingLoss
from cldice.clDice.cldice_loss.pytorch.cldice import soft_cldice as cldice_loss
from skeleton_recall import SoftSkeletonRecallLoss, cldiceSkeletonLoss

from utils.mask_gen_3D import CutMix3D
import time
...
parser.add_argument('--root_dir', type=str, default="../data/Dataset337_aneart", help='Path to BraTS-2019 dataset')
parser.add_argument('--patch_size', type=str, default=[96, 96, 96], help='Input image patch size')

parser.add_argument('--exp', type=str, default='Dataset337_aneart', help='Experiment name')
...
parser.add_argument('--num_workers', type=int, default=4, help='train num workers')

parser.add_argument('--labelnum', type=int, default=35, help='Number of labeled samples per class')
...
parser.add_argument('--alpha', type=float, default=1.0, help='Weight for topo_criterions loss')
parser.add_argument('--beta', type=float, default=1.0, help='Weight for topo_criterions loss')
...
if __name__ == "__main__":
    ...
    # Read dataset
    db_train = IAVS(base_dir=args.root_dir, 
                         split='train', 
                         transform=T.Compose([
                             SagittalToAxial(),
    ...
    trainloader = DataLoader(db_train, batch_sampler=batch_sampler, num_workers=args.num_workers, pin_memory=True, worker_init_fn=worker_init_fn)
    ...
    topo_criterions = [#{'loss': BettiMatchingLoss(num_processes=16, use_base_loss=False),   'weight': 0.05, 'name': 'BettiMatchingLoss'},
                       #{'loss': HutopoLoss(num_processes=16, use_base_loss=False),          'weight': 0.01, 'name': 'HutopoLoss'}, 
                       #{'loss': cldice_loss(),                                              'weight': 1.00, 'name': 'CLDiceLoss'},
                       #{'loss': SoftSkeletonRecallLoss(),                                   'weight': 1.00, 'name': 'SoftSkeletonRecallLoss'},
                       {'loss': cldiceSkeletonLoss(alpha=args.alpha, beta=args.beta),        'weight': 1.00, 'name': 'cldiceSkeletonLoss'},
                       #{'loss':MosinLoss(use_base_loss=False),                              'weight': 0.05, 'name': 'MosinLoss'},
                       #{'loss':TopographLoss(num_processes=16, use_base_loss=False),        'weight': 0.05, 'name': 'TopographLoss'},
                       #{'loss':WarpingLoss(use_base_loss=False),                            'weight': 0.05, 'name': 'WarpingLoss'},
                       ]

    topo_time = []

    mixxer = CutMix3D(mix_ratio=0.5)
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # CutMix
            #volume_batch[:labeled_bs], label_batch[:labeled_bs], _ = mixxer.apply_cutmix(volume_batch[:labeled_bs], label_batch[:labeled_bs])
            #volume_batch[labeled_bs:], label_batch[labeled_bs:], _ = mixxer.apply_cutmix(volume_batch[labeled_bs:], label_batch[labeled_bs:])
            ...
            loss_seg_ce = F.cross_entropy(stud_logits[:labeled_bs], label_batch[:labeled_bs]) # 0.9020
            ...
            ema_labels = torch.argmax(ema_probs, dim=1, keepdim=True)
            loss_semi_topo = torch.tensor(0., device=loss_seg_dice.device)
            for tc in topo_criterions:
                loss_semi_topo += tc['weight'] * tc['loss'](stud_probs[labeled_bs:, 1:2, :, :, :], ema_labels[labeled_bs:])
            end_time = time.time()
            topo_time.append(end_time - start_time)

            # Gather losses
            #loss = args.l_weight * (loss_seg_ce + loss_seg_dice) + consistency_weight * consistency_loss + args.u_weight * (f_loss + u_loss)
            loss = args.l_weight * (loss_seg_ce + loss_seg_dice) + consistency_weight * (consistency_loss + loss_semi_topo) + args.u_weight * (f_loss + u_loss)
            ...
            writer.add_scalar('info/loss_topo', loss_semi_topo, iter_num)
            ...
            # logging.info(
            #     'Iteration %d : Loss : %03f, Loss_CE: %03f, Loss_Dice: %03f, Loss_Topo: %03f, UnCLoss: %03f, FeCLoss: %03f, mean_dice: %03f, mean_hd95: %03f' %
            #     (iter_num, loss.item(), loss_seg_ce.item(), loss_seg_dice.item(), loss_semi_topo.item(), u_loss.item(), f_loss.item(), dice_score.mean().item(), np.mean(hausdorff_score).item()))

            logging.info(
                'Iteration %d : Loss : %03f, Loss_CE: %03f, Loss_Dice: %03f, Loss_Topo: %03f, UnCLoss: %03f, FeCLoss: %03f' %
                (iter_num, loss.item(), loss_seg_ce.item(), loss_seg_dice.item(), loss_semi_topo.item(), u_loss.item(), f_loss.item()))
            
            if iter_num > 0 and iter_num % 200 == 0:
                model.eval()
                avg_metric = test_3d_patch.var_all_case_IAVS(model, args.root_dir, num_classes=args.num_classes, patch_size=patch_size, stride_xy=64, stride_z=64)
